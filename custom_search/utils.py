...
def simplify_indexes(indexes):
    ...
    i = 1
    simplified_index_start = sorted_indexes[0][0]
    simplified_index_end = sorted_indexes[0][1]
    while True:
        index = sorted_indexes[i]
        if index[0] <= simplified_index_end:
            simplified_index_end = max(simplified_index_end, index[1])
        else:
            simplified_indexes.append((simplified_index_start, simplified_index_end))
            simplified_index_start = index[0]
            simplified_index_end = index[1]
